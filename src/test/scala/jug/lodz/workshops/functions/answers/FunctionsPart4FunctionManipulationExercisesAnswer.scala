...
/**
  ...
  */
class FunctionsPart4FunctionManipulationExercisesAnswer extends FunSuite with Matchers {
  ...
  private def createDecrement(): Int => Int = i => i - 1
  ...
  def injectParser(parser: ParsingStrategyServiceBean): String => Int = s => parser(s) - 1
  ...
  def decorateWithNullCheck(f: String => Int,default:String) : String=>Int =
      s=> if(s==null) f(default) else f(s)
  ...
  def reduce[A](l:List[A])(f:(A,A)=>A): A ={
    ...
    for(e <- l.tail) result = f(result,e)
  ...
  test("domain example"){
    ...
    val lightPolicy : Policy = cash => cash >10
    val longTermBusiness : BusinessLogic = cash => cash + Math. ceil(cash * 0.1).toInt

    val bankWithLightPolicy = cashService(lightPolicy) _
    val longTermfinancialService = bankWithLightPolicy(longTermBusiness)
