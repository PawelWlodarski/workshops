...
import Robots3Answer._

object Robots3Answer {
  ...
  object PunchGenerator{
    ...
    def const(value:Int):PunchGenerator = new ConstantPunchGenerator(value)
    def random(upTo:Int):PunchGenerator = new RandomPuchGenerator(upTo)
  }
  ...
  object Shield{
    ...
    private class ChargedShield(initialEnergy:Int) extends Shield{
      ...
      override def absorb(hit: Int): Int =
    ...
    private object NoShield extends Shield{
      override def absorb(hit: Int): Int = hit
    }


    def createShield(energy:Int):Shield = new ChargedShield(energy)

    def noShield():Shield=NoShield
  }
...
class Robot3Test extends FunSuite with MustMatchers {
  test("No shield is not protecting at all"){
    Robots3Answer.Shield.noShield().absorb(10) mustBe 10
    Robots3Answer.Shield.noShield().absorb(10) mustBe 10
    Robots3Answer.Shield.noShield().absorb(10) mustBe 10
  }

  test("Standard shield should absorb hits"){
    val shield=Robots3Answer.Shield.createShield(100)
  ...
  test("robot should generate punch between 0 and 50"){
    val robot = Robots3Answer.Robot3(100,Robots3Answer.Shield.noShield(),PunchGenerator.const(10))
  ...
  test("shield absorbs energy"){
    val robot=Robots3Answer.Robot3(70,Robots3Answer.Shield.createShield(10),PunchGenerator.const(10))
  ...
  test("robots battle"){
    val robot1=Robots3Answer.Robot3(70,Robots3Answer.Shield.createShield(50),PunchGenerator.const(20))
    val robot2=Robots3Answer.Robot3(70,Robots3Answer.Shield.createShield(50),PunchGenerator.const(10))

    Robots3Answer.Robot3.battle(robot1,robot2)
  ...
  test("Robot with positive energy should be alive") {
    Robots3Answer.Robot3.createDefault(20).isAlive mustBe true
    Robots3Answer.Robot3.createDefault(1).isAlive mustBe true
    Robots3Answer.Robot3.createDefault(200).isAlive mustBe true
  }

  test("Robot initial energy should be greater than 0") {
    an[IllegalArgumentException] must be thrownBy Robots3Answer.Robot3(0,Robots3Answer.Shield.noShield(),PunchGenerator.const(10))
    an[IllegalArgumentException] must be thrownBy Robots3Answer.Robot3(-1,Robots3Answer.Shield.noShield(),PunchGenerator.const(10))
    an[IllegalArgumentException] must be thrownBy Robots3Answer.Robot3(-100,Robots3Answer.Shield.noShield(),PunchGenerator.const(10))
  }

  test("hit robot"){
    val robot=Robots3Answer.Robot3.createDefault(100)
