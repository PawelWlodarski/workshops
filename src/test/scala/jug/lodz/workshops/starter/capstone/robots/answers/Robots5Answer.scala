...
import jug.lodz.workshops.starter.capstone.robots.answers.Robots5Answer._
import org.scalatest.{FunSuite, MustMatchers}

import scala.util.Random

object Robots5Answer {
...
class Robot5AnswerTest extends FunSuite with MustMatchers {

  test("Game should be played tille the end"){
    val robot1=Robots5Answer.Robot5(70,Robots5Answer.Shield.createShield(15),PunchGenerator.const(10))
    val robot2=Robots5Answer.Robot5(70,Robots5Answer.Shield.createShield(10),PunchGenerator.const(30))
  ...
  test("No shield is not protecting at all"){
    Robots5Answer.Shield.noShield().absorb(10) mustBe 10
    Robots5Answer.Shield.noShield().absorb(10) mustBe 10
    Robots5Answer.Shield.noShield().absorb(10) mustBe 10
  }

  test("Standard shield should absorb hits"){
      val shield=Robots5Answer.Shield.createShield(100)
  ...
  test("robot should generate punch between 0 and 50"){
    val robot = Robots5Answer.Robot5(100,Robots5Answer.Shield.noShield(),PunchGenerator.const(10))
  ...
  test("shield absorbs energy"){
    val robot=Robots5Answer.Robot5(70,Robots5Answer.Shield.createShield(10),PunchGenerator.const(10))
  ...
  test("robots battle"){
    val robot1=Robots5Answer.Robot5(70,Robots5Answer.Shield.createShield(50),PunchGenerator.const(20))
    val robot2=Robots5Answer.Robot5(70,Robots5Answer.Shield.createShield(50),PunchGenerator.const(10))

    Robots5Answer.Game.battle(robot1,robot2)
  ...
  test("Robot with positive energy should be alive") {
    Robots5Answer.Robot5.createDefault(20).isAlive() mustBe true
    Robots5Answer.Robot5.createDefault(1).isAlive() mustBe true
    Robots5Answer.Robot5.createDefault(200).isAlive() mustBe true
  }

  test("Robot initial energy should be greater than 0") {
    an[IllegalArgumentException] must be thrownBy Robots5Answer.Robot5(0,Robots5Answer.Shield.noShield(),PunchGenerator.const(10))
    an[IllegalArgumentException] must be thrownBy Robots5Answer.Robot5(-1,Robots5Answer.Shield.noShield(),PunchGenerator.const(10))
    an[IllegalArgumentException] must be thrownBy Robots5Answer.Robot5(-100,Robots5Answer.Shield.noShield(),PunchGenerator.const(10))
  }

  test("hit robot"){
    val robot=Robots5Answer.Robot5.createDefault(100)
