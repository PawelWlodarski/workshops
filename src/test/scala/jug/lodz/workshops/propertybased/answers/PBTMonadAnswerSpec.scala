...
/**
  ...
  */
class PBTMonadAnswerSpec extends PropSpec with Matchers with PropertyChecks{
  ...
  property("Option satisfies left identity law : M.unit(a).flatMap(f) == f(a)"){
    forAll{(li:List[Int])=>
      Option(li).flatMap(largest) shouldBe(largest(li))
    }
  }

  property("Option satisfies right identity law : M.flatMap(M.unit) == M"){
    forAll{(oi:Option[Int])=>
      oi.flatMap(i=>Option(i)) shouldBe(oi)
    }
  }


  property("Option satisfies Associativity law : M.flatMap(f1).flatMap(f2) == M.flatMap(x=>f1(x).flatMap(f2))"){
    forAll(optionalListGenerator){oli=>
      oli.flatMap(largest).flatMap(findPurchaseInMockDB) shouldBe(oli.flatMap(ints=>largest(ints).flatMap(findPurchaseInMockDB)))
    }
  }
