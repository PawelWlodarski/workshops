...
/**
  ...
  */
object FP2ReadingFile1Answer {

  def demonstration(l:List[Int]):List[Int]= l.filter(e=>e>7)

  //exercise
  def filter_1(l:List[Int]):List[Int]= l.filter(_ % 2 ==0)

  //exercise
  val someSet=Set(3,7,10,50)
  def filter_2(l:List[Int]):List[Int]= l.filter(e => someSet.contains(e))

  //additional
  def additional(l:List[Int]): (List[Int], List[Int]) = l.partition(e=>e % 2==0)
  ...
  def readFile(): Unit = {
    ...
    try {
      source.getLines()
        .filter(line=>line.split(",")(0) == "user1")
        .foreach(println)
    } finally {
      source.close()
    }
