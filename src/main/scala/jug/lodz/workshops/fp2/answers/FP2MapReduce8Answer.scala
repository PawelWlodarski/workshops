...
object FP2MapReduce8Answer {
  ...
  object MapReduce{
    def apply[A,B,C](fmap:A=>B)(reduce:List[B]=>C) : List[A] => C =
      liftGeneric(fmap) andThen reduce
  }
  ...
  lazy val extractProduct:String => String = line => line.split(",")(1)

  val countOccurences: (List[String]) => Map[String, Int] =fold(Map.empty[String,Int]){ (acc:Map[String,Int], elem:String)=>
    val currentValue = acc.getOrElse(elem,0)
    acc + (elem -> (currentValue+1))
  }
