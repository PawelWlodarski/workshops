...
/**
  ...
  */
object EffectsPart2UnderstandOptionAnswer {
  ...
  def main(args: Array[String]) {
    ...
    val html2 = UsersDAO.find(2).map(userToHTML).fold(displayError("user with id 2 not found"))(displayPage)

    println("[ID=2] - one line with domain transformation")
    UsersDAO.find(2).map(userToHTML).map(displayPage).orElse(Some(displayError("user with id 2 not found"))).foreach(println)
    val html3 = UsersDAO.find(2).map(userToHTML).map(displayPage).getOrElse(displayError("user with id 2 not found"))
