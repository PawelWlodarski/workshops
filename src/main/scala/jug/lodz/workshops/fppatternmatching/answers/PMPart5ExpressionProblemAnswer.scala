...
/**
  ...
  */
object PMPart5ExpressionProblemAnswer {
  ...
  object ExerciseLevelMonad {
    ...
    sealed trait RightBiasedEither[+A, +B] {
      def map[C](f: B => C): RightBiasedEither[A, C] = this match {
        case left@RBLeft(_) => left
        case RBRight(b) => RBRight(f(b))
      }

      def flatMap[AA >: A, C](f: B => RightBiasedEither[AA, C]): RightBiasedEither[AA, C] = this match {
        case left@RBLeft(_) => left
        case RBRight(b) => f(b)
      }
    }
